...
function setModalAttr(id) {
    ...
    for (const [key, value] of Object.entries(liConf)) {
        ...
        if (typeof value === "boolean") {
            ...
            if (key === "text" && (["date", "name", "code"].includes(liConf["class"]))) {
...
function initBox() {
    backend.getConf('f10a_supplement', setBoxChildren);
}
    ...
    for (let i = 0; i < 3; i++) {
        ...
        switch (p['tag']) {
            case 'text':
                ...
                break;
            case 'date':
                $('.row-header').append(`<li id="${i}" tag="date" style="font-size:${p['font_size']}px;font-weight: ${p['bold'] ? 'bold' : ''};background-color:${p['bg_color']}" class="block">${p['display'] ? p['text'].slice(p['format']) : ''}</li>`);
                break;
        }
    }
    let box = $("#dragBox");
...
function saveConfAndReload(reload = true) {
    //reorder
    let ids = $('#dragBox li').map(function () {
        return $(this).attr('id');
    }).get();
    ids.unshift('2');
    ids.unshift('1');
    ...
    //pass to backend
    backend.saveConf("f10a_supplement", confList, async function (n) {
        if (n) {
            let toast = $('#toast');
            toast.modal('show');
...
function setQuitListener() {
    $("#home").click(function () {
        backend.nav("home", "f10a_supplement");
    });
}
...
function setResetListener() {
    $("#reset").click(function () {
        ...
        if (confirm("Confirm to reset?")) {
            backend.resetConf('f10a_supplement', function (l) {
                confList = l;
                document.location.reload();
            });
        }
